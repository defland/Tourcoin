...
def check_etourates(node, fees_seen, max_invalid, print_etourates = True):
    '''
    This function calls etouratefee and verifies that the etourates
    meet certain invariants.
    '''
    all_etourates = [ node.etouratefee(i) for i in range(1,26) ]
    if print_etourates:
        print([str(all_etourates[e-1]) for e in [1,2,3,6,15,25]])
    delta = 1.0e-6 # account for rounding error
    last_e = max(fees_seen)
    for e in [x for x in all_etourates if x >= 0]:
        # Etourates should be within the bounds of what transactions fees actually were:
        if float(e)+delta < min(fees_seen) or float(e)-delta > max(fees_seen):
            raise AssertionError("Etourated fee (%f) out of range (%f,%f)"
                                 %(float(e), min(fees_seen), max(fees_seen)))
        # Etourates should be monotonically decreasing
        if float(e)-delta > last_e:
            raise AssertionError("Etourated fee (%f) larger than last fee (%f) for lower number of confirms"
                                 %(float(e),float(last_e)))
        last_e = e
    valid_etourate = False
    invalid_etourates = 0
    for i,e in enumerate(all_etourates): # etourate is for i+1
        if e >= 0:
            valid_etourate = True
            # etouratesmartfee should return the same result
            assert_equal(node.etouratesmartfee(i+1)["feerate"], e)

        else:
            invalid_etourates += 1

            # etouratesmartfee should still be valid
            approx_etourate = node.etouratesmartfee(i+1)["feerate"]
            answer_found = node.etouratesmartfee(i+1)["blocks"]
            assert(approx_etourate > 0)
            assert(answer_found > i+1)

            # Once we're at a high enough confirmation count that we can give an etourate
            # We should have etourates for all higher confirmation counts
            if valid_etourate:
                raise AssertionError("Invalid etourate appears at higher confirm count than valid etourate")

    # Check on the expected number of different confirmation counts
    # that we might not have valid etourates for
    if invalid_etourates > max_invalid:
        raise AssertionError("More than (%d) invalid etourates"%(max_invalid))
    return all_etourates


class EtourateFeeTest(BitcoinTestFramework):
    ...
    def setup_network(self):
        ...
        # so the etourates would not be affected by the splitting transactions
        ...
        self.nodes.append(start_node(1, self.options.tmpdir,
                                     ["-blockprioritysize=1500", "-blockmaxsize=17000",
                                      "-maxorphantx=1000", "-relaypriority=0", "-debug=etouratefee"]))
    ...
    def run_test(self):
        ...
        print("Will output etourates for 1/2/3/6/15/25 blocks")

        for i in xrange(2):
            print("Creating transactions and mining them with a block size that can't keep up")
            # Create transactions and mine 10 small blocks with node 2, but create txs faster than we can mine
            self.transact_and_mine(10, self.nodes[2])
            check_etourates(self.nodes[1], self.fees_per_kb, 14)

            print("Creating transactions and mining them at a block size that is just big enough")
            # Generate transactions while mining 10 more blocks, this time with node1
            # which mines blocks with capacity just above the rate that transactions are being created
            self.transact_and_mine(10, self.nodes[1])
            check_etourates(self.nodes[1], self.fees_per_kb, 2)
        ...
        sync_blocks(self.nodes[0:3],.1)
        print("Final etourates after emptying mempools")
        check_etourates(self.nodes[1], self.fees_per_kb, 2)

if __name__ == '__main__':
    EtourateFeeTest().main()
