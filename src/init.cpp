...
#endif
bool fFeeEtouratesInitialized = false;
...
static const char* FEE_ETOURATES_FILENAME="fee_etourates.dat";
...
void PrepareShutdown()
{
    ...
    if (fFeeEtouratesInitialized)
    {
        boost::filesystem::path est_path = GetDataDir() / FEE_ETOURATES_FILENAME;
        CAutoFile est_fileout(fopen(est_path.string().c_str(), "wb"), SER_DISK, CLIENT_VERSION);
        if (!est_fileout.IsNull())
            mempool.WriteFeeEtourates(est_fileout);
        else
            LogPrintf("%s: Failed to write fee etourates to %s\n", __func__, est_path.string());
        fFeeEtouratesInitialized = false;
    }
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-fallbackfee=<amt>", strprintf(_("A fee rate (in %s/kB) that will be used when fee etouration has insufficient data (default: %s)"),
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, mempoolrej, net, proxy, prune, http, libevent, tor, zmq, "
                             "tour (or specifically: privatesend, instantsend, masternode, spork, keepass, mnpayments, gobject)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
...
std::string LicenseInfo()
{
    ...
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The Tour Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-fallbackfee"))
    {
        ...
        if (nFeePerK > nHighTransactionFeeWarning)
            InitWarning(_("-fallbackfee is set very high! This is the transaction fee you may pay when fee etourates are not available."));
    ...
    if (!est_filein.IsNull())
        mempool.ReadFeeEtourates(est_filein);
    fFeeEtouratesInitialized = true;
    ...
    // ********************************************************* Step 11d: start tour-privatesend thread
