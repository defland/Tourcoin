...
QString CoinControlDialog::getPriorityLabel(double dPriority, double mempoolEtouratePriority)
{
    double dPriorityMedium = mempoolEtouratePriority;
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        // Priority
        double mempoolEtouratePriority = mempool.etourateSmartPriority(nTxConfirmTarget);
        dPriority = dPriorityInputs / (nBytes - nBytesInputs + (nQuantityUncompressed * 29)); // 29 = 180 - 151 (uncompressed public keys are over the limit. max 151 bytes of the input are ignored for priority)
        sPriorityLabel = CoinControlDialog::getPriorityLabel(dPriority, mempoolEtouratePriority);
        ...
        // Allow free? (require at least hard-coded threshold and default to that if no etourate)
        double dPriorityNeeded = std::max(mempoolEtouratePriority, AllowFreeThreshold());
    ...
    // how many satoshis the etourated fee can vary per byte we guess wrong
    double dFeeVary;
    if (payTxFee.GetFeePerK() > 0)
        dFeeVary = (double)std::max(CWallet::GetRequiredFee(1000), payTxFee.GetFeePerK()) / 1000;
    else {
        dFeeVary = (double)std::max(CWallet::GetRequiredFee(1000), mempool.etourateSmartFee(nTxConfirmTarget).GetFeePerK()) / 1000;
    }
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            itemOutput->setText(COLUMN_PRIORITY, CoinControlDialog::getPriorityLabel(dPriority, mempoolEtouratePriority));
        ...
        if (treeMode)
        {
            ...
            itemWalletAddress->setText(COLUMN_PRIORITY, CoinControlDialog::getPriorityLabel(dPrioritySum, mempoolEtouratePriority));
