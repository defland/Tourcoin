...
void SendCoinsDialog::updateFeeSectionControls()
{
    ...
    ui->labelFeeEtouration      ->setEnabled(ui->radioSmartFee->isChecked());
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    int etourateFoundAtBlocks = nBlocksToConfirm;
    CFeeRate feeRate = mempool.etourateSmartFee(nBlocksToConfirm, &etourateFoundAtBlocks);
    if (feeRate <= CFeeRate(0)) // not enough data => minfee
    {
        ui->labelSmartFee->setText(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
                                                                std::max(CWallet::fallbackFee.GetFeePerK(), CWallet::GetRequiredFee(1000))) + "/kB");
        ui->labelSmartFee2->show(); // (Smart fee not initialized yet. This usually takes a few blocks...)
        ui->labelFeeEtouration->setText("");
    }
    else
    {
        ui->labelSmartFee->setText(BitcoinUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
        ...
        ui->labelFeeEtouration->setText(tr("Etourated to begin confirmation within %n block(s).", "", etourateFoundAtBlocks));
    }
