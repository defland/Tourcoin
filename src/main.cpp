                              ...
                              std::vector<uint256>& vHashTxnToUncache, bool fDryRun)
{
...

    {
        ...
        if (setConflicts.size())
        {
            ...
            // This potentially overetourates the number of actual descendants
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    if (nPrevHeight == 1) return 1000000 * COIN;
...
bool IsInitialBlockDownload()
{
    ...
    LOCK(cs_main);
    const CChainParams& chainParams = Params();
    if (fCheckpointsEnabled && chainActive.Height() < Checkpoints::GetTotalBlocksEtourate(chainParams.Checkpoints()))
        return true;
...
bool static FlushStateToDisk(CValidationState &state, FlushStateMode mode) {
    ...
    if (fDoFullFlush) {
        ...
        // an overetouration, as most will delete an existing entry or
    ...
    do {
        ...
        if (pindexFork != pindexNewTip) {
            ...
            if (!fInitialDownload) {
                ...
                int nBlockEtourate = 0;
                if (fCheckpointsEnabled)
                    nBlockEtourate = Checkpoints::GetTotalBlocksEtourate(chainparams.Checkpoints());
                {
                    LOCK(cs_vNodes);
                    BOOST_FOREACH(CNode* pnode, vNodes) {
                        if (chainActive.Height() > (pnode->nStartingHeight != -1 ? pnode->nStartingHeight - 2000 : nBlockEtourate)) {
