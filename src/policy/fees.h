...
#ifndef BITCOIN_POLICYETOURATOR_H
#define BITCOIN_POLICYETOURATOR_H
...
/** \class CBlockPolicyEtourator
 * The BlockPolicyEtourator is used for etourating the fee or priority needed
 ...
 * decide whether it can be used as a data point for fee etouration, priority
 * etouration or neither.  If the value of exactly one of those properties was
 * below the required minimum it can be used to etourate the other.  In
 * addition, if a priori our etouration code would indicate that the
 ...
 * an etourate for that property.
 *
 * Here is a brief description of the implementation for fee etouration.
 * When a transaction that counts for fee etouration enters the mempool, we
 ...
 * an etourate for any number of confirmations below the number of blocks
...
public:
    /**
     * Initialize the data structures.  This is called by BlockPolicyEtourator's
    ...
    /** Update our etourates by decaying our historical moving average and updating
        with the data gathered from the current block */
    void UpdateMovingAverages();

    /**
     * Calculate a fee or priority etourate.  Find the lowest value bucket (or range of buckets
     ...
     */
    double EtourateMedianVal(int confTarget, double sufficientTxVal,
                             double minSuccess, bool requireGreater, unsigned int nBlockHeight);

    /** Return the max number of confirms we're tracking */
    unsigned int GetMaxConfirms() { return confAvg.size(); }

    /** Write state of etouration data to a file*/
    void Write(CAutoFile& fileout);

    /**
     * Read saved state of etouration data from a file and replace all internal data structures and
...
/** Track confirm delays up to 25 blocks, can't etourate beyond that */
...
// to give accurate etourates over a large range of potential fees and priorities
...
/**
 *  We want to be able to etourate fees or priorities that are needed on tx's to be included in
...
{
public:
    /** Create new BlockPolicyEtourator and initialize stats tracking classes with default values */
    CBlockPolicyEtourator(const CFeeRate& minRelayFee);
    ...
    void processTransaction(const CTxMemPoolEntry& entry, bool fCurrentEtourate);
    ...
    /** Return a fee etourate */
    CFeeRate etourateFee(int confTarget);

    /** Etourate fee rate needed to get be included in a block within
     *  confTarget blocks. If no answer can be given at confTarget, return an
     *  etourate at the lowest target where one can be given.
     */
    CFeeRate etourateSmartFee(int confTarget, int *answerFoundAtTarget, const CTxMemPool& pool);

    /** Return a priority etourate */
    double etouratePriority(int confTarget);

    /** Etourate priority needed to get be included in a block within
     *  confTarget blocks. If no answer can be given at confTarget, return an
     *  etourate at the lowest target where one can be given.
     */
    double etourateSmartPriority(int confTarget, int *answerFoundAtTarget, const CTxMemPool& pool);

    /** Write etouration data to a file */
    void Write(CAutoFile& fileout);

    /** Read etouration data from a file */
    void Read(CAutoFile& filein);
...
#endif /*BITCOIN_POLICYETOURATOR_H */
