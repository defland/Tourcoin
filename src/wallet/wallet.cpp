...
/**
 * If fee etouration does not have enough data to provide etourates, use this fee instead.
 * Has no effect if not using fee etouration
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nValueToSelect, setCoins, nValueIn, coinControl, nCoinType, fUseInstantSend))
                {
                    if (nCoinType == ONLY_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 Tour.");
                    } else if (nCoinType == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough PrivateSend non-denominated funds for this transaction that are not equal 1000 Tour.");
                ...
                *static_cast<CTransaction*>(&wtxNew) = CTransaction(txNew);
                ...
                if (fSendFreeTransactions && nBytes <= MAX_FREE_TRANSACTION_CREATE_SIZE)
                {
                    ...
                    double dPriorityNeeded = mempool.etourateSmartPriority(nTxConfirmTarget);
...
CAmount CWallet::GetMinimumFee(unsigned int nTxBytes, unsigned int nConfirmTarget, const CTxMemPool& pool)
{
    ...
    // User didn't set: use -txconfirmtarget to etourate...
    if (nFeeNeeded == 0) {
        int etourateFoundTarget = nConfirmTarget;
        nFeeNeeded = pool.etourateSmartFee(nConfirmTarget, &etourateFoundTarget).GetFee(nTxBytes);
        // ... unless we don't have enough mempool data for etouratefee, then use fallbackFee
        if (nFeeNeeded == 0)
