 ...
 * If 'height' is nonnegative, compute the etourate at the time when a given block was found.
 */
...
UniValue getnetworkhashps(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "\nReturns the etourated network hashes per second based on the last n blocks.\n"
            "Pass in [blocks] to override # of blocks, -1 specifies since last difficulty change.\n"
            "Pass in [height] to etourate the network speed at the time when a certain block was found.\n"
            "\nArguments:\n"
            "1. blocks     (numeric, optional, default=120) The number of blocks, or -1 for blocks since last difficulty change.\n"
            "2. height     (numeric, optional, default=-1) To etourate at the time of the given height.\n"
            "\nResult:\n"
            "x             (numeric) Hashes per second etourated\n"
            "\nExamples:\n"
...
UniValue etouratefee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "etouratefee nblocks\n"
            "\nEtourates the approximate fee per kilobyte needed for a transaction to begin\n"
            "confirmation within nblocks blocks.\n"
            "\nArguments:\n"
            "1. nblocks     (numeric)\n"
            "\nResult:\n"
            "n              (numeric) etourated fee-per-kilobyte\n"
            "\n"
            "A negative value is returned if not enough transactions and blocks\n"
            "have been observed to make an etourate.\n"
            "\nExample:\n"
            + HelpExampleCli("etouratefee", "6")
            );
    ...
    CFeeRate feeRate = mempool.etourateFee(nBlocks);
...
UniValue etouratepriority(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "etouratepriority nblocks\n"
            "\nEtourates the approximate priority a zero-fee transaction needs to begin\n"
            "confirmation within nblocks blocks.\n"
            "\nArguments:\n"
            "1. nblocks     (numeric)\n"
            "\nResult:\n"
            "n              (numeric) etourated priority\n"
            "\n"
            "A negative value is returned if not enough transactions and blocks\n"
            "have been observed to make an etourate.\n"
            "\nExample:\n"
            + HelpExampleCli("etouratepriority", "6")
            );
    ...
    return mempool.etouratePriority(nBlocks);
}

UniValue etouratesmartfee(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "etouratesmartfee nblocks\n"
            "\nWARNING: This interface is unstable and may disappear or change!\n"
            "\nEtourates the approximate fee per kilobyte needed for a transaction to begin\n"
            "confirmation within nblocks blocks if possible and return the number of blocks\n"
            "for which the etourate is valid.\n"
            "\nArguments:\n"
            "1. nblocks     (numeric)\n"
            "\nResult:\n"
            "{\n"
            "  \"feerate\" : x.x,     (numeric) etourate fee-per-kilobyte (in BTC)\n"
            "  \"blocks\" : n         (numeric) block number where etourate was found\n"
            "}\n"
            "\n"
            "A negative value is returned if not enough transactions and blocks\n"
            "have been observed to make an etourate for any number of blocks.\n"
            "However it will not return a value below the mempool reject fee.\n"
            "\nExample:\n"
            + HelpExampleCli("etouratesmartfee", "6")
            );
    ...
    UniValue result(UniValue::VOBJ);
    int answerFound;
    CFeeRate feeRate = mempool.etourateSmartFee(nBlocks, &answerFound);
    result.push_back(Pair("feerate", feeRate == CFeeRate(0) ? -1.0 : ValueFromAmount(feeRate.GetFeePerK())));
    result.push_back(Pair("blocks", answerFound));
    return result;
}

UniValue etouratesmartpriority(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "etouratesmartpriority nblocks\n"
            "\nWARNING: This interface is unstable and may disappear or change!\n"
            "\nEtourates the approximate priority a zero-fee transaction needs to begin\n"
            "confirmation within nblocks blocks if possible and return the number of blocks\n"
            "for which the etourate is valid.\n"
            ...
            "\nResult:\n"
            "{\n"
            "  \"priority\" : x.x,    (numeric) etourated priority\n"
            "  \"blocks\" : n         (numeric) block number where etourate was found\n"
            "}\n"
            "\n"
            "A negative value is returned if not enough transactions and blocks\n"
            ...
            "\nExample:\n"
            + HelpExampleCli("etouratesmartpriority", "6")
            );
    ...
    UniValue result(UniValue::VOBJ);
    int answerFound;
    double priority = mempool.etourateSmartPriority(nBlocks, &answerFound);
