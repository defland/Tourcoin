    ...
    nTransactionsUpdated(0)
{
    ...
    minerPolicyEtourator = new CBlockPolicyEtourator(_minReasonableRelayFee);
    minReasonableRelayFee = _minReasonableRelayFee;
}

CTxMemPool::~CTxMemPool()
{
    delete minerPolicyEtourator;
}
...
bool CTxMemPool::addUnchecked(const uint256& hash, const CTxMemPoolEntry &entry, setEntries &setAncestors, bool fCurrentEtourate)
{
    ...
    minerPolicyEtourator->processTransaction(entry, fCurrentEtourate);
...
void CTxMemPool::removeUnchecked(txiter it)
{
    ...
    mapTx.erase(it);
    nTransactionsUpdated++;
    minerPolicyEtourator->removeTx(hash);
...
/**
 * Called when a block is connected. Removes from mempool and updates the miner fee etourator.
 */
void CTxMemPool::removeForBlock(const std::vector<CTransaction>& vtx, unsigned int nBlockHeight,
                                std::list<CTransaction>& conflicts, bool fCurrentEtourate)
{
    LOCK(cs);
    ...
    // After the txs in the new block have been removed from the mempool, update policy etourates
    minerPolicyEtourator->processBlock(nBlockHeight, entries, fCurrentEtourate);
...
CFeeRate CTxMemPool::etourateFee(int nBlocks) const
{
    LOCK(cs);
    return minerPolicyEtourator->etourateFee(nBlocks);
}
CFeeRate CTxMemPool::etourateSmartFee(int nBlocks, int *answerFoundAtBlocks) const
{
    LOCK(cs);
    return minerPolicyEtourator->etourateSmartFee(nBlocks, answerFoundAtBlocks, *this);
}
double CTxMemPool::etouratePriority(int nBlocks) const
{
    LOCK(cs);
    return minerPolicyEtourator->etouratePriority(nBlocks);
}
double CTxMemPool::etourateSmartPriority(int nBlocks, int *answerFoundAtBlocks) const
{
    LOCK(cs);
    return minerPolicyEtourator->etourateSmartPriority(nBlocks, answerFoundAtBlocks, *this);
}

bool
CTxMemPool::WriteFeeEtourates(CAutoFile& fileout) const
{
    try {
        LOCK(cs);
        fileout << 120000; // version required to read: 0.12.00 or later
        fileout << CLIENT_VERSION; // version that wrote the file
        minerPolicyEtourator->Write(fileout);
    }
    catch (const std::exception&) {
        LogPrintf("CTxMemPool::WriteFeeEtourates(): unable to write policy etourator data (non-fatal)\n");
        return false;
    }
    return true;
}
...
bool
CTxMemPool::ReadFeeEtourates(CAutoFile& filein)
{
    try {
        int nVersionRequired, nVersionThatWrote;
        filein >> nVersionRequired >> nVersionThatWrote;
        if (nVersionRequired > CLIENT_VERSION)
            return error("CTxMemPool::ReadFeeEtourates(): up-version (%d) fee etourate file", nVersionRequired);

        LOCK(cs);
        minerPolicyEtourator->Read(filein);
    }
    catch (const std::exception&) {
        LogPrintf("CTxMemPool::ReadFeeEtourates(): unable to read policy etourator data (non-fatal)\n");
        return false;
    }
    return true;
}
...
size_t CTxMemPool::DynamicMemoryUsage() const {
    LOCK(cs);
    // Etourate the overhead of mapTx to be 12 pointers + an allocation, as no exact formula for boost::multi_index_contained is implemented.
...
bool CTxMemPool::addUnchecked(const uint256&hash, const CTxMemPoolEntry &entry, bool fCurrentEtourate)
{
    LOCK(cs);
    setEntries setAncestors;
    uint64_t nNoLimit = std::numeric_limits<uint64_t>::max();
    ...
    return addUnchecked(hash, entry, setAncestors, fCurrentEtourate);
}
